	...
	if(unique==0){
		...
		$.ajax({
			...
			success:function(data){
				unique = 0;
				if(data==1){
					$.messager.alert('提示',"保存成功","success",function(){reloadPage(contextPath + '/group/groupManager?op_menu=10',100);});
				}else{
	...
	if(unique==0){
		...
		$.ajax({
			...
			success:function(data){
				unique = 0;
				if(data==1){
					$.messager.alert('提示',"删除成功!","success",function(){reloadPage(contextPath + '/group/groupManager?op_menu=10',100);});
				}else{
					$.messager.alert('提示',"操作失败！","error");
				}
			},
