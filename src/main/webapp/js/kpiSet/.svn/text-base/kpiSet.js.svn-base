var minVal=[-130,-30,-30,0,0,0];
var maxVal=[0,30,0,150,1000,100];
var kpiName=['RSRP','SNR','RSRQ','下行链路带宽','延时','丢包'];
...
$(function(){
	...
	$("#updateBtn").live('click',function(){
		...
		var min = minVal[$("#kpiType_page").val()-1];
		var max = maxVal[$("#kpiType_page").val()-1];
		var minKpi = $("#kpiRangVal11").val();
		var maxKpi = $("#kpiRangVal62").val();
		if(minKpi<min){
			$("#modal-backdrop").hide();
			$("#kpiRangVal11").parent('div').addClass("has-error");
			$.messager.alert('提示',kpiName[$("#kpiType_page").val()-1]+"的最小值至少为"+min+kpiUnit[$("#kpiType_page").val()-1],"error");
		}else if(maxKpi>max){
			$("#modal-backdrop").hide();
			$("#kpiRangVal62").parent('div').addClass("has-error");
			$.messager.alert('提示',kpiName[$("#kpiType_page").val()-1]+"的最大值不超过"+max+kpiUnit[$("#kpiType_page").val()-1],"error");
		}else if(flag){
			saveKpi(dt);
		}else{
			$("#modal-backdrop").hide();
